...
void visualization(int n, Robot robot, int step, Robot p[], Robot pr[])
{
    ...
    plt::save("/home/rtk/git_projects/rse/project3/exercise3/RoboND-MCL-Lab/Images/" + to_string(step) + ".png");
    plt::clf();
}

int main()
{
    /*std::cout<<"I'm ready to code MCL"<<std::endl;
    ...
    R_obj.set(10, 10, 0);
    string pose;
    pose = R_obj.show_pose();
    std::cout<<"The robot is located at "<<pose<<std::endl;
    
    // Moving the robot
    R_obj.move(M_PI/2, 10);
    pose = R_obj.show_pose();
    std::cout<<"The robot is located at "<<pose<<std::endl;
    
    // Print distances from the landmarks
    string landmark_dist;
    landmark_dist = R_obj.read_sensors();
    std::cout<<"Distance from landmarks "<<landmark_dist<<std::endl;

    // Adding noise to the pose and sensor values
    R_obj.set_noise(5.0, 0.1, 5.0);

    R_obj.move(0, 10);
    
    pose = R_obj.show_pose();
    std::cout<<"The robot is located at (noise is added)"<<pose<<std::endl;
    landmark_dist = R_obj.read_sensors();
    std::cout<<"Distance from landmakrs (noise is added) "<<landmark_dist<<std::endl;
    */
    ...
    int n_p = 1000;
    Robot R_Particles[n_p];
    //std::vector<Robot> R_Particles;
    //R_Particles.assign(n_p, Robot()); 
    // VECTOR WILL NOT CREATE RANDOM PARTICLES
    for (int i = 0; i < n_p; i++)
    {
        // Assign random noise
        R_Particles[i].set_noise(0.05, 0.05, 5.0);
        // std::cout<<R_Particles[i].show_pose()<<std::endl;
    }
    ...
    for(int t=0; t<50; t++){
        ...
        //std::cout<<R_obj.show_pose()<<std::endl;

        //************************** assign weights to the paticles based on the measurements
        double w[n_p];
        for (int i = 0; i < n_p; i++)
        {
            w[i] = R_Particles[i].measurement_prob(z);
            //std::cout<<w[i]<<std::endl;
        }

        //************************** Resampling wheel algorthim
        // resampled particles
        Robot p3[n_p];
        
        // pick a random particle
        int index = gen_real_random() * n_p;
        double beta = 0.0;
        double mw = max(w, n_p);
        ...
        for (int i = 0; i < n_p; i++) {
            ...
            while (beta > w[index]) {
                beta -= w[index];
                index = mod((index + 1), n_p);
            }
            p3[i] = R_Particles[index];
        }
        
        // storing resampled particles as the particles for the next iteraion
        for (int k=0; k < n_p; k++) {
            P2[k] = p3[k];
        ...
        std::cout << "Step = " << t << ", Evaluation = " << evaluation(R_obj, P2, n_p) << std::endl;
        visualization(n_p, R_obj, t, R_Particles, P2);
    }
